...
def convert_to_coco_api(ds: Dataset) -> COCO:
    """
    Convert pytorch dataset to COCO api.
...
def get_coco_api_from_dataset(dataset: Dataset) -> COCO:
    """
    Get COCO api object from pytorch dataset.
...
def get_iou_types(model: nn.Module) -> List[str]:
    """
    Get supported target types for IoU metric evaluation.
